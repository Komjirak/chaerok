...
export function Save() {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = await fetch(`${CLOUD_FUNCTIONS_URL}/analyze`, {
        method: 'POST',
      ...
      <div className="max-w-md mx-auto px-6 py-6">
        ...
        {loading ? null : !user ? (
          <Msg
            ...
            body={
              isEn
                ? 'Use the same account as your iPhone and it lands in the same notes.'
                : '아이폰 채록에서 쓰던 계정으로 로그인하면 같은 생각 노트에 담겨요.'
            }
          >
          ...
          <Msg
            ...
            title={isEn ? 'Part of Chaerok Mind' : '채록 Mind에서 쓸 수 있어요'}
            body={
              isEn
                ? 'Saving from the web and picking it up on your iPhone is a Mind feature.'
                : '웹에서 담은 기록을 계정에 보관하고 아이폰에서 이어 보는 건 Mind의 기능이에요.'
            }
          >
