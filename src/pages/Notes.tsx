      ...
      <main className="flex-1 bg-surface-paper grid place-items-center px-5 py-24">
        <div className="max-w-sm text-center">
          ...
          <p className="text-ink-muted mb-8 leading-relaxed">
            {isEn
              ? 'Pick up where you left off — the notes you captured on your iPhone, here.'
              : '아이폰 채록에 담은 기록을 여기서 이어 보세요.'}
          </p>
          ...
          <p className="mt-5 text-xs text-ink-muted leading-relaxed">
            {isEn
              ? 'Sign in only matters if you keep notes on your account. Use the same account as your iPhone.'
              : '로그인은 기록을 계정에 묶어둘 때만 필요해요. 아이폰에서 쓰던 계정으로 로그인해 주세요.'}
          </p>
        </div>
      </main>
    );
        ...
        <section>
          ...
          {notes === null ? (
            ...
            <Empty
              ...
              body={
                isEn
                  ? 'On your iPhone, check Settings → Organizing → Process in the cloud. Once it is on, your notes appear here.'
                  : '아이폰 채록에서 설정 → 정리하는 곳 → "클라우드에서 처리"가 켜져 있는지 확인해 주세요. 켜져 있으면 잠시 뒤 올라와요.'
              }
            />
          ) : (
            <Empty
              ...
              body={
                isEn
                  ? 'Keeping notes on your account is part of Chaerok Mind. Turn on cloud processing on your iPhone and they show up here.'
                  : '기록을 계정에 보관하는 건 채록 Mind의 기능이에요. 아이폰에서 클라우드 처리를 켜면 여기에 나타나요.'
              }
              action={
                <Link to="/#pricing">
