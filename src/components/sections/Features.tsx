        ...
        <div className="grid md:grid-cols-3 gap-8">
          {features.map((feature, idx) => (
            <motion.div
              ...
              transition={{ duration: 0.4, delay: idx * 0.15, type: "spring", stiffness: 300, damping: 20 }}
              className={`rounded-2xl p-8 shadow-ambient border border-surface-amber/50 ${feature.bgColor} transition-shadow hover:shadow-xl`}
            >
              <motion.div 
                className={`w-14 h-14 rounded-full flex items-center justify-center mb-6 ${feature.iconColor}`}
